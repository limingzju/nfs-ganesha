    ...
    void *ptr)
{
	...
	LogDebug(COMPONENT_FSAL, " entry count=%ld", count);
	...
	for (i=0; i< count; i++) {
		...
		fsal_release_attrs(&attrs);
                ...
                LogDebug(COMPONENT_FSAL, " cb cache inode err:%d", err);
    ...
    void *dir_state, fsal_readdir_cb cb, attrmask_t attrmask, bool *eof)
{
	...
	myself = container_of(dir_hdl, struct nedge_fsal_obj_handle,
	    obj_handle);
	LogDebug(COMPONENT_FSAL, "%ju whence=%s", h2inode(myself), (char*)whence);
	...
	retval = FNEDGE_ERRMAP(ccow_fsio_readdir_cb4(myself->ci,
	    h2inode(myself), nedge_readdir_cb4, (char *)whence, (void *)&dat, eof));
