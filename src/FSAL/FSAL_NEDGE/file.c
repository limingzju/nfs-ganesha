...
void
nedge_read2(struct fsal_obj_handle *obj_hdl, bool bypass, fsal_async_cb done_cb,
    struct fsal_io_arg *read_arg, void *caller_arg)
{
	...
	for (int i = 0; i < read_arg->iov_count; i++) {
		retval = FNEDGE_ERRMAP(ccow_fsio_read(my_fd.file, offset,
		    read_arg->iov[i].iov_len, read_arg->iov[i].iov_base, &nb_read, &eof));

		read_arg->end_of_file = (bool)eof;
...
void
nedge_write2(struct fsal_obj_handle *obj_hdl, bool bypass, fsal_async_cb done_cb,
    struct fsal_io_arg *write_arg, void *caller_arg)
{
	...
	for (int i = 0; i < write_arg->iov_count; i++) {
		retval = FNEDGE_ERRMAP(ccow_fsio_write(my_fd.file, offset,
		    write_arg->iov[i].iov_len, write_arg->iov[i].iov_base,
		    &nb_written));
